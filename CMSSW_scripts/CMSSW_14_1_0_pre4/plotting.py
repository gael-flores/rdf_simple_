...
def CreateAxisHist(src, at_limits=True):
    ...
    if (at_limits):
        ...
        for i in range(1, src.GetN()):
            src.GetPoint(i, x, y)
            if float(x.value) < min:
                min = float(x.value)
            if float(x.value) > max:
                max = float(x.value)
        #result.GetXaxis().SetLimits(min, max)
        result.GetXaxis().SetLimits(0.1, max)
    R.gPad = backup
    return result
...
def DrawLimitBand(pad, graph_dict, draw=['exp2', 'exp1', 'exp0', 'obs'], draw_legend=None,
    ...
    legend_dict = {
        ...
        'exp1' : { 'Label' : '#pm68% Expected', 'LegendStyle' : 'F', 'DrawStyle' : '3SAME'},
        'exp2' : { 'Label' : '#pm95% Expected', 'LegendStyle' : 'F', 'DrawStyle' : '3SAME'}
    }
