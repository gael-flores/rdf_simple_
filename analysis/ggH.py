...
def save_report(df, report_name, sample, opts):
...
def ggH(data,phi_mass,sample):
    ...
    for mass in phi_mass:
        ggH3g=ggH3g.Define('Photon_corrIso_m{}'.format(mass),'correct_gammaIso(Photon_pt,Photon_eta,Photon_phi,Photon_pfRelIso03_all,Photon_preselection)')
        ...
        ggH3g=ggH3g.Define('non_MC_cut_m{}'.format(mass),'sample_isMC==0 && best_3g_raw_mass_m{m}<30|best_3g_raw_mass_m{m}>140 && best_3g_sumID_m{m}==3'.format(m=mass))
        

    #at least 4 photons
    ggH4g=ggH.Filter('Sum(Photon_preselection==1)>3','at_least_4_preselected_photons')
    for mass in phi_mass:
        ggH4g=ggH4g.Define('Photon_corrIso_m{}'.format(mass),'correct_gammaIso(Photon_pt,Photon_eta,Photon_phi,Photon_pfRelIso03_all,Photon_preselection)')
    ...
    save_report(ggH3g, "Report_ggH3g", f"{sample}_ggH3g", opts)
    save_report(ggH4g, "Report_ggH4g", f"{sample}_ggH4g", opts)
