...
#    ggH=ggH.Filter('Sum(genPhotonDR<0.2)>2','At least three matched photons')
    ...
    ggH3g=ggH3g.Filter('best_3g_phi_valid_m30==1')
    ...
    h3_phi = ggH3g.Histo1D(("best_3g_phi_mass_m30", "m_phi;m_{#phi} (GeV);N_{Events}", 15, 0, 100),"best_3g_phi_mass_m30")
    h3_dxy  = ggH3g.Histo1D(("best_3g_phi_dxy_m30", "Best 3#gamma: calculated d_{xy};d_{xy}(cm);N_{Events}", 20, -50,150),"best_3g_phi_dxy_m30")
    '''
    #at least 4 photons
    ...
    ggH4g=ggH4g.Filter('best_4g_phi1_valid_m30==1 && best_4g_phi2_valid_m30==1')
    ...
    h4_phi1_dxy  = ggH4g.Histo1D(("best_4g_phi1_dxy_m30", "Best 4#gamma: calculated d_{xy};d_{xy}(cm);N_{Events}", 20, -50,150),"best_4g_phi1_dxy_m30")
    h4_phi2_dxy  = ggH4g.Histo1D(("best_4g_phi2_dxy_m30", "Best 4#gamma: calculated d_{xy};d_{xy}(cm);N_{Events}", 20, -50,150),"best_4g_phi2_dxy_m30")
    h4_phi1 = ggH4g.Histo1D(("best_4g_phi1_mass_m30", "m_phi;m_{#phi} (GeV);N_{Events}", 15, 0, 100),"best_4g_phi1_mass_m30")
    h4_phi2 = ggH4g.Histo1D(("best_4g_phi2_mass_m30", "m_phi;m_{#phi} (GeV);N_{Events}", 15, 0, 100),"best_4g_phi2_mass_m30")
    ...
    c3=ROOT.TCanvas("canvas3", "", 1200, 900)
    h3_dxy.Draw()
    h3_dxy.SetLineColor(3)
    c3.Draw()
    c3.SaveAs('3photon_dxy.png')

    c4=ROOT.TCanvas("canvas4","",1200,900)
    h4_phi2_dxy.Draw()
    h4_phi2_dxy.SetLineColor(3)
    h4_phi1_dxy.Draw("Same")
    h4_phi1_dxy.SetLineColor(4)
    leg=ROOT.TLegend(0.5, 0.7, 0.6, 0.6)
    leg.AddEntry("best_4g_phi1_dxy_m30","#phi_{1}","f")
    leg.AddEntry("best_4g_phi2_dxy_m30","#phi_{2}","f")
    leg.SetTextSize(0.05)
    leg.SetBorderSize(0)
    leg.Draw("Same")
    c4.Draw()
    c4.SaveAs('4photon_dxy.png')

    c5=ROOT.TCanvas("canvas5","",1200,900)
    h4_phi2.Draw()
    h4_phi2.SetLineColor(4)
    h4_phi1.Draw("Same")
    h4_phi1.SetLineColor(3)
    leg5=ROOT.TLegend(0.5, 0.7, 0.6, 0.6)
    leg5.AddEntry("best_4g_phi1_mass_m30","#phi_{1}","f")
    leg5.AddEntry("best_4g_phi2_mass_m30","#phi_{2}","f")
