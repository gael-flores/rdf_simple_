...
def compareSideband(loose, tight, m, name, header):
    ...
    upper.cd()
    if drawprelim:
        CMS_lumi.CMS_lumi(upper, 4, 0, relPosX=0.077, lumi_13TeV = lumifb[options.era], extraText = "Work in progress")
    c.cd()
...
def compareSignal(background, signal0, signal100, m, name, header, blind= False):
    ...
    if drawprelim:
        CMS_lumi.CMS_lumi(c, 4, 0, relPosX=0.077, lumi_13TeV = lumifb[options.era], extraText = "Simulation Work in progress")
...
def compareAll(background, data, signal0, signal100, m, name, header, blind= False):
...
for v in ['Z']:
    for l in ['ELE', 'MU']:
        ...
        sig0 = f.Get("HToPhiPhi_ctau0")
        sig0.Scale(0.1)
        sig100 = f.Get("HToPhiPhi_ctau100")
        sig100.Scale(0.1)

        compareSignal(loose, sig0, sig100, m, "closure_{}H_{}_m{}_signal_{}".format(v,l,m,options.era), "Signal Region, "+headers[v][l]+" m_{#Phi}="+"{} GeV".format(m))
