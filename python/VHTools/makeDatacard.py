...
parser.add_option("-v", "--v", dest = "v", default = 'W', help = "V category (Z or W)")
...
def unfoldRebin(histogram,name,binsX:array.array):
    unfolded = startVH.unfoldTH2(histogram)
    unfolded.Rebin(len(binsX) - 1, name, binsX)
    rebinned = ROOT.gDirectory.Get(name)
    return(rebinned)

def data_histogram_setup(binsM:int, binsLxy:int, lxyMin:float, lxyMax:float, v:str,l:str,m:int) -> Callable:
    def make_histogram(name:str,appliedCuts:str):
        histogram = dataEMU.hist2d(
...
def monteCarlo_histogram_setup(appliedCuts:str, binsM:int, binsLxy:int, lxyMin:float, lxyMax:float, v:str, l:str, m:int) -> Callable:
...
def WJets_MC_histogram_setup(appliedCuts:str, binsM:int, binsLxy:int, lxyMin:float, lxyMax:float, v:str, l:str, m:int) -> Callable:
    def make_channel_histogram(mc_plotter, name:str, scaleFactors:list[str]):
        histogram = mc_plotter[startVH.ana[v][l]].hist2d(
            f"best_2g_raw_mass_m{m}",
            f"best_2g_dxy_m{m}",
            f"({appliedCuts})*({scaleFactors})", 
            startVH.lumi[era], 
...
def DY_MC_histogram_setup(appliedCuts:str, binsM:int, binsLxy:int, lxyMin:float, lxyMax:float, v:str, l:str, m:int) -> Callable:
    def make_channel_histogram(name:str, scaleFactors:list[str]):
        histogram = dy_plotter[startVH.ana[v][l]].hist2d(
            f"best_2g_raw_mass_m{m}",
            f"best_2g_dxy_m{m}",
            f"({appliedCuts})*({scaleFactors})", 
            startVH.lumi[era], 
...
def processChannels(name,scaleFactors,ct,appliedCuts,binsX,histogram_suffix = '',correction="") -> dict[str]:
    print("\nhistogram name: ",name)
    sig = {}
    channels_to_remove = []
    global channels
...
def processLeptonScaleFactors(l:str, ct:int, v:str, binsX) -> tuple[str]:
    ...
    for scaleFactor in list_of_scaleFactors:
        ...
        sigUp   = processChannels(f"sig_{lepton_lowerCase}{scaleFactor}Up_{v}_{l}_m{m}_ct{ct}",  sfUp,   ct,final_selection_cuts, binsX, histogram_suffix = f"_{lepton_lowerCase}_{scaleFactor}Up")
        sigDown = processChannels(f"sig_{lepton_lowerCase}{scaleFactor}Down_{v}_{l}_m{m}_ct{ct}",sfDown, ct,final_selection_cuts, binsX, histogram_suffix = f"_{lepton_lowerCase}_{scaleFactor}Down")
...
def processPhotonScaleFactors(factors_toProcess,ct):
    ...
    for scaleFactor in factors_toProcess:
        ...
        sigUp:dict[str]   = processChannels(f"sig_pho{scaleFactor}Up_{v}_{l}_m{m}_ct{ct}",  sfUp,   ct,final_selection_cuts,binsX ,histogram_suffix = f"_pho_{scaleFactor}Up")
        sigDown:dict[str] = processChannels(f"sig_pho{scaleFactor}Down_{v}_{l}_m{m}_ct{ct}",sfDown, ct,final_selection_cuts,binsX ,histogram_suffix = f"_pho_{scaleFactor}Down")
...
def processPhotonCorrections(corrections:list[str], ct:int) -> tuple[dict[str, dict[str, ROOT.TH1D]] , dict[str, dict[str, ROOT.TH1D]]]:
    # Energy scale/smearing corrections, scale factor corrections
    sigScaleUp   = {}
    sigScaleDown = {}
    for correction in corrections:
        ...
        sigUp   = processChannels(f"sig_pho{correction}Up_{v}_{l}_m{m}_ct{ctau}",  sfs, ct,final_selection_cuts,binsX,  histogram_suffix = f"_pho_{correction}Up",correction=f"_{correction}Up")
        sigDown = processChannels(f"sig_pho{correction}Down_{v}_{l}_m{m}_ct{ctau}",sfs, ct,final_selection_cuts,binsX,  histogram_suffix = f"_pho_{correction}Down",correction=f"_{correction}Down")

        for channel in channels:
            sigScaleUp[correction][channel]:ROOT.TH1D   = fillZeroBins(sigUp[channel], 0)
            sigScaleDown[correction][channel]:ROOT.TH1D = fillZeroBins(sigDown[channel], 0)
...
if __name__ == "__main__":
    try:
        ...
        dataEMU = startVH.getDataPlotters(era,prod,"/store/user/gfavila/DDP",[m],startVH.ana[v][l],modelIndependent=modelIndependent)
        signal  = startVH.getSigPlotters(era,prod,"/store/user/gfavila/DDP",[m],startVH.ana[v][l],modelIndependent=modelIndependent,which_lifetime=1000)['signal']        
        dy_plotter  = startVH.getDYPlotters(era,prod,"/store/user/gfavila/DDP",[m],startVH.ana[v][l],modelIndependent=modelIndependent,which_mc=['DYJetsToLL_M50_LO','DYJetsToLL_M10'])['DY']
        wj_plotter    = startVH.getWJPlotters(era,prod,"/store/user/gfavila/DDP",[m],startVH.ana[v][l],modelIndependent=modelIndependent,which_mc=['WJetsToLNu_HT','TTJets'])['WJets']
        wg_plotter    = startVH.getWJPlotters(era,prod,"/store/user/gfavila/DDP",[m],startVH.ana[v][l],modelIndependent=modelIndependent,which_mc=['WGToLNuG'])['WJets']
        wgg_plotter   = startVH.getWJPlotters(era,prod,"/store/user/gfavila/DDP",[m],startVH.ana[v][l],modelIndependent=modelIndependent,which_mc=['WGG'])['WJets']
        ttgg_plotter  = startVH.getWJPlotters(era,prod,"/store/user/gfavila/DDP",[m],startVH.ana[v][l],modelIndependent=modelIndependent,which_mc=['TTGG'])['WJets']
        ...
        loose_sb = dataEMU.hist1d("best_2g_dxy_m{}".format(m), cuts['cr'][v][l][m]+"&&best_2g_raw_mass_m{}>65".format(m), "1", (f"loose_sb_{v}_{l}_m{m}", "", 50, -1400, 0))
        tight_sb = dataEMU.hist1d("best_2g_dxy_m{}".format(m), cuts['sr'][v][l][m]+"&&best_2g_raw_mass_m{}>65".format(m), "1", (f"loose_sb_{v}_{l}_m{m}", "", 50, -1400, 0))
        closure_sb = dataEMU.hist1d("best_2g_dxy_m{}".format(m), cuts['cl'][v][l][m]+"&&best_2g_raw_mass_m{}>65".format(m), "1",(f"closure_sb_{v}_{l}_m{m}", "", 50, -1400, 0))
        ...
        make_data_histogram = data_histogram_setup(binsM, binsLxy, lxyMin, lxyMax, v,l,m)
        ...
        background = unfoldRebin(background, "data_loose_rebinned",binsX)
        background.Scale(scale)
        print(f"Background Integral: {background.Integral()}")
        ...
        closure_2bad = unfoldRebin(closure_2bad, "closure_loose_rebinned",binsX)
        ...
        closure_1bad = unfoldRebin(closure_1bad,"closure_tight_rebinned",binsX)
        ...
        make_dy_histogram = DY_MC_histogram_setup(final_selection_cuts,binsM,binsLxy,lxyMin,lxyMax,v,l,m)
        make_wj_histogram = WJets_MC_histogram_setup(final_selection_cuts,binsM,binsLxy,lxyMin,lxyMax,v,l,m) #sr cuts
        
        wg_histogram_sr = make_wj_histogram(wg_plotter, f"WG_sr_{v}_{l}_{m}", sfs)
        wg_histogram_sr = unfoldRebin(wg_histogram_sr, f"WG_sr_{v}_{l}_{m}_rebinned",binsX)
        wg_histogram_sr.Write(f"WG_sr_{v}_{l}_{m}", ROOT.TObject.kOverwrite)
        print("WG integral: ",wg_histogram_sr.Integral())

        dy_histogram_sr = make_dy_histogram(f"DY_sr_{v}_{l}_{m}", sfs) #this is now a 2d histogram with 2 id photons
        dy_histogram_sr = unfoldRebin(dy_histogram_sr, f"DY_sr_{v}_{l}_{m}",binsX)
        dy_histogram_sr.Write(f"DY_sr_{v}_{l}_{m}", ROOT.TObject.kOverwrite)
        print("DY integral: ",dy_histogram_sr.Integral())

        wj_histogram_sr = make_wj_histogram(wj_plotter, f"WJets_sr_{v}_{l}_{m}", sfs)
        wj_histogram_sr = unfoldRebin(wj_histogram_sr, f"WJets_sr_{v}_{l}_{m}",binsX)
        wj_histogram_sr.Write(f"WJets_sr_{v}_{l}_{m}", ROOT.TObject.kOverwrite)
        print("WJ integral: ",wj_histogram_sr.Integral())

        wgg_histogram_sr = make_wj_histogram(wgg_plotter, f"WGG_sr_{v}_{l}_{m}", sfs)
        wgg_histogram_sr = unfoldRebin(wgg_histogram_sr, f"WGG_sr_{v}_{l}_{m}_rebinned",binsX)
        wgg_histogram_sr.Write(f"WGG_sr_{v}_{l}_{m}", ROOT.TObject.kOverwrite)
        print("WGG integral: ",wgg_histogram_sr.Integral())

        ttgg_histogram_sr = make_wj_histogram(ttgg_plotter, f"TTGG_sr_{v}_{l}_{m}", sfs)
        ttgg_histogram_sr = unfoldRebin(ttgg_histogram_sr, f"TTGG_sr_{v}_{l}_{m}_rebinned",binsX)
        ttgg_histogram_sr.Write(f"TTGG_sr_{v}_{l}_{m}", ROOT.TObject.kOverwrite)
        print("ttGG integral: ",ttgg_histogram_sr.Integral())
        make_histogram = monteCarlo_histogram_setup(final_selection_cuts,binsM,binsLxy,lxyMin,lxyMax,v,l,m)
        ...
        for ctau in lifetimes:
            ...
            # Nominal signal
            sig = processChannels(f"sig_{v}_{l}_m{m}_ct{ct}",sfs,ctau,final_selection_cuts,binsX)

            sigScaleUp = {}
            sigScaleDown = {}

            Lepton_sigScaleUp, Lepton_sigScaleDown = processLeptonScaleFactors(l,ctau,v,binsX)
